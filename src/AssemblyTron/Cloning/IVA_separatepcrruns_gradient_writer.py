    ...
    if x in section['parts'].values:
        ...
        for i, row in GG_dfs.iterrows():
            ...
            for i, row in globals()[x].iterrows():
                f.write(
                    "    right_pipette.pick_up_tip(tiprack1['"+str(tiprackposition[t300])+"']) \r\n"
                    "    right_pipette.aspirate(50, pcrplate['"+str(globals()[x].loc[i].at['frag_loc'])+"']) \r\n"
                    "    right_pipette.dispense(50, secondarydils['"+str(globals()[x].loc[i].at['frag_loc'])+"']) \r\n"
                    "    right_pipette.blow_out() \r\n"
                    "    right_pipette.drop_tip() \r\n"
                )
                t300+=1
        ...
        for i, row in GG_dfs.iterrows():
            ...
            for i, row in globals()[x].iterrows():
                if globals()[x].loc[i].at['final amount to add'] < 10:
                    ...
                    f.write(
                        "    right_pipette.pick_up_tip(tiprack1['"+str(tiprackposition[t300])+"']) \r\n"
                        "    right_pipette.aspirate("+str(globals()[x].loc[i].at['final amount to add'])+", secondarydils['"+str(globals()[x].loc[i].at['frag_loc'])+"']) \r\n"
                        "    right_pipette.dispense("+str(globals()[x].loc[i].at['final amount to add'])+", pcrplate['"+str(globals()[x].loc[i].at['location_of_assembly'])+"']) \r\n"
                        "    right_pipette.blow_out() \r\n"
                        "    right_pipette.drop_tip() \r\n"
                    )
                    t300+=1