        ...
        for i, row in df.iterrows():
            if df.loc[i].at['water to add'] > 8:
                f.write(
                    ...
                    "    left_pipette.aspirate("+str(df.loc[i].at['amount of template to add'])+", "+str(globals()[df.loc[i].at['rack']])+"['"+str(df.loc[i].at['template_origin'])+"'], rate=1.0) \r\n"
            ...
            if df.loc[i].at['water to add'] < 8:
                f.write(
                    "    left_pipette.pick_up_tip() \r\n"
                    "    left_pipette.aspirate("+str(3*(df.loc[i].at['amount of template to add']))+", "+str(globals()[df.loc[i].at['rack']])+"['"+str(df.loc[i].at['template_origin'])+"'], rate=1.0) \r\n"
