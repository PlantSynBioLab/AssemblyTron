...
def run(protocol: protocol_api.ProtocolContext): 
    ...
    right_pipette.pick_up_tip() 
    right_pipette.aspirate(8.75, watertuberack['A1'], rate=2.0) 
    right_pipette.dispense(8.75, pcrplate['H2'], rate=2.0) 
    right_pipette.blow_out() 
    right_pipette.drop_tip() 
    ...
    left_pipette.pick_up_tip() 
    left_pipette.aspirate(1.0, tuberack2['B1'], rate=2.0) 
    left_pipette.dispense(1.0, pcrplate['H2'], rate=2.0) 
    left_pipette.mix(3,2,pcrplate['H2']) 
    left_pipette.drop_tip() 
    left_pipette.pick_up_tip() 
    left_pipette.aspirate(1.0, tuberack2['B2'], rate=2.0) 
    left_pipette.dispense(1.0, pcrplate['H2'], rate=2.0) 
    ...
    left_pipette.dispense(1.0, pcrplate['H2'], rate=2.0) 
    left_pipette.mix(3,3,pcrplate['H2']) 
    left_pipette.drop_tip() 
    left_pipette.pick_up_tip() 
    ...
    left_pipette.pick_up_tip() 
    left_pipette.aspirate(0.75, tuberack2['D6'], rate=2.0) 
    left_pipette.dispense(0.75, pcrplate['H2'], rate=2.0) 
    left_pipette.blow_out() 
    left_pipette.drop_tip() 
    right_pipette.pick_up_tip() 
    ...
    right_pipette.pick_up_tip() 
    right_pipette.aspirate(12.5, cold_tuberack['D6'], rate=2.0) 
    right_pipette.aspirate(12.5, pcrplate['H2'], rate=2.0) 
    right_pipette.blow_out() 
    right_pipette.drop_tip() 
    right_pipette.pick_up_tip() 
    ...
    right_pipette.drop_tip() 
    right_pipette.pick_up_tip() 
    right_pipette.mix(3,15.5,pcrplate['H2']) 
    right_pipette.blow_out() 
    right_pipette.drop_tip() 
    tc_mod.deactivate() 
    temp_module.deactivate() 
    protocol.pause('move to gradient thermocycler. set gradiet to be between 63.907042970080354 and 68.7110220009685. Extension time should be 31.740000000000002 seconds. Follow normal parameters for everything else. A1 is cool, A8 is hot.') 
    temp_module.set_temperature(4) 
