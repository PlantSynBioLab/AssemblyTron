...
#path for my imac
path = "/data/user_storage/oligo.csv"
...
df['Template Concentration'] = [126] #all you have to do is input the template concentrations in the right order
...
#path for my windows machine
path = '/data/user_storage/combinations.csv'

combinations = pandas.read_csv(path)
combinations
...
#maybe use the median annealing temperature in the spreadsheet
    ...
    #NEED TO ADD A TIME CALCULATION FOR EXTENSION AND TEMP FOR ANNEALING
    tc_mod.close_lid()
...
#Now add DPNI for digestion
    for i, row in combinations.iterrows():
        right_pipette.pick_up_tip()
        right_pipette.aspirate(DPNI, cold_tuberack['A1'], rate=2.0)
        right_pipette.aspirate(DPNI, pcrplate[combinations.loc[i].at['pcrwell']], rate=2.0)
        right_pipette.drop_tip()
