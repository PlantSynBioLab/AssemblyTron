#' Parse AFB Epistasis j5 assembly csv file into python readable feathers
#' 
#' Originally written by Ryan Smith
#' Adapted for AFB Epistasis by John Bryant
#'
...
parse_j5 <- function(path = getwd(), file = "_condensed.csv")
{
  ...
  combinations_read <- readr::read_csv(file, col_names = c("ID Number", "Name","Aseembly Method", "Part(s) Bin 0", "Assembly Piece ID Number Bin 0", "Part(s) Bin 1", "Assembly Piece ID Number Bin 1"),skip = combinations+2)
