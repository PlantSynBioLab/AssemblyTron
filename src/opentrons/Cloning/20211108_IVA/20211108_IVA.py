...
combinations['water to add'] = (Input_values.loc[0].at['pcrvol']-primersadded- tempsadded - Q5)
...
########NEED TO TAKE OUT THE FIRST AND LAST ROWS

loc_i = pandas.read_csv(paths.loc[0].at['opentrons_repo']+'/output_new_pcrwell_location.csv')
...
newlocation_df.to_csv(paths.loc[0].at['opentrons_repo']+'/output_new_pcrwell_location.csv')
...
#add 1uL of each primer
    for i, row in combinations.iterrows():
        ...
        for j in filter_col:
            ...
            left_pipette.mix(3,4,pcrplate[combinations.loc[i].at['pcrwell']])
            #left_pipette.blow_out()
            left_pipette.drop_tip()
...
#add 1uL of each template
    for i, row in combinations.iterrows():
        ...
        for j in temp_col:
            ...
            left_pipette.dispense(combinations.loc[i].at['amount primer to add to IVA'], pcrplate[combinations.loc[i].at['pcrwell']], rate=2.0)
            left_pipette.mix(3,5,pcrplate[combinations.loc[i].at['pcrwell']])
            #left_pipette.blow_out()
            left_pipette.drop_tip()
