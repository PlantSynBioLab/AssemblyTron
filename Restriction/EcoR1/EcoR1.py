...
os.chdir("/data/user_storage/Restriction/EcoR1")
os.getcwd()

plasmid = pd.read_csv("20220408/20220408_ecor1.csv",engine = 'python', encoding='utf-8-sig')
...
plasmid['Volume of Water'] = 50 - plasmid['Volume of Plasmid'] - plasmid['Buffer'] - plasmid['EcoR1']
...
# pick up PME1 -> dispense into pcr tube -> get rid of tip
    ...
    for i, row in plasmid.iterrows():
        pipette_left.pick_up_tip()
        pipette_left.aspirate(plasmid.loc[i].at['EcoR1'],cold_tuberack['A4'],2)
        pipette_left.dispense(plasmid.loc[i].at['EcoR1'],pcr[plasmid.loc[i].at['Digestion Tube']],2)
        pipette_left.mix(3,plasmid.loc[i].at['EcoR1'],pcr[plasmid.loc[i].at['Digestion Tube']])
        pipette_left.blow_out()
        pipette_left.drop_tip()
